...
- 依据 Toonflow 自身的 A/B/C/D 评分、问题清单与建议方案审批：A 或修复后 B 只有在本轮问题清单中不存在“严重程度：严重”的审核项时才通过；仍有严重项时逐项采用其后对应的 Toonflow 原生建议方案继续修复并复审；首次 B 仍按原报告修复；C/D 继续按原报告修复并复审；多选方案取报告列出的第一个方案；
...
默认配置 `"requireSevereIssueResolution": true`。控制器只在 Toonflow 本轮“问题清单”范围内识别明确字段 `严重程度：严重`，不会因普通正文出现“严重”二字而拦截。只要 A/B 报告仍有严重项，就不放行；若严重项后缺少可关联的原生“建议方案”，则暂停并报告，不自行编造修复内容。每次复审都重新读取新的问题清单，不沿用上一轮已经消除的严重项。
